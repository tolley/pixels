window.addEventListener( 'load', () => {
    ...
    emailAllowed.addEventListener("click", function() {
        fetch( '/toggle-emails-allowed' ).then( async ( resp ) => {
    ...
    async function fetchViewport() {
        ...
        try {
            const res  = await fetch(`image-api?x1=${x1}&y1=${y1}&x2=${x2}&y2=${y2}`);
    ...
    async function apiDelete(region) {
        const url = region
            ? `/image-api?x1=${region.x1}&y1=${region.y1}&x2=${region.x2}&y2=${region.y2}`
            : '/image-api';
        try {
    ...
    function applyZoom(z) {
        ...
        recalc();
        scheduleViewportFetch();
        // Tolley if (cellPx !== prev) gtag('event', 'zoom_change', { zoom_px: cellPx });
    }
