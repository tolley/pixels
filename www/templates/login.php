...
<div class="card">
    ...
    <button type="button" id="btn-resend" class="btn-link"
            style="<?= !is_null( $showResend ) ? '' : 'display:none' ?>">Resend verification email</button>
    ...
    <p class="footer-link"><a href="signup">Don't have an account? Sign up</a></p>
</div>

<script>
    if( <?= json_encode( $initError ) ?> )
        var INIT_ERROR  = <?= json_encode($initError) ?>;
    else
        var INIT_ERROR = '';
    ...
    btnResend.addEventListener('click', async () => {
        ...
        try {
            const res  = await fetch('auth?action=resend', {
                method:  'POST',
                headers: { 'Content-Type': 'application/json' },
